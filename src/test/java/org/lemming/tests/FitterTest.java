...
public class FitterTest<T extends RealType<T> & NativeType<T>> {
	...
	public FrameElements<T> setUp() {
		// load input image
		final String filename = "/Users/ronny/ownCloud/test.tif";
	...
	public static <T extends RealType<T> & NativeType<T>> void main(String[] args) {
		...
		// Gaussian
		AbstractModule mf = new GaussianFitter<T>(6);
		mf.setOutput(store);
		mf.processData(el);
		...
		// Quadratic 
		AbstractModule qf = new QuadraticFitter<T>(6);
		qf.setOutput(store);
		qf.processData(el);
		...
		while(!store.isEmpty()) 
			System.out.println(store.poll().toString());
		// Symmetric Gaussian 
		AbstractModule sf = new SymmetricGaussianFitter<T>(6);
		sf.setOutput(store);
		sf.processData(el);
		...
		while(!store.isEmpty()) 
			System.out.println(store.poll().toString());
		// M2LE Java 
		AbstractModule gf = new GradientFitter<T>(6,LemmingUtils.readCSV("/Users/ronny/ownCloud/set1-calb.csv"));
		gf.setOutput(store);
		gf.processData(el);
